...
- 개인 상세 = 기존 `app/(untabs)/community/[id]`(개인입양 상세) **재활용+수정**
  - **하단 CTA 소유자 분기**: 비소유자=문의하기(기존) / 소유자=**"입양완료 처리"**(입양중)·"입양중으로 변경"(완료) — 기존 BottomButton 재활용, 소유자엔 무의미한 문의하기 대체
  - **완료 뱃지** prominent 표시(상세·카드)
  - ⋮ 메뉴에 상태 변경 항목(보조)
- 분기 = S1 카드 `onPress`가 `source`에 따라 공공/개인 상세 라우트로
- 라우트 경로·공유 URL은 §6 Open Issue
...
- `profile-activity-scene` [수정] — 내 활동의 개인입양 카테고리 라벨 → **"내 공고"**, 카드 ⋮ 메뉴(기존 수정/삭제)에 **"입양완료로 변경"** 추가
- `profile-like-scene` [수정] — 찜 "공고" 탭을 공공+개인 통합 (개인 공고 찜 추가로 가능)
- **신규 없음** (라벨·메뉴 항목·구성)
...
| 개인 공고 찜   | 생략 / favorite 추가                 | **favorite 추가(마이그레이션)**            | 공고 카드 시그니처가 찜. 공공·개인 일관 필요(사용자 지정)                        |
| 개인 공고 상태 | 삭제로 처리 / status 추가            | **status(입양중/완료) 추가(마이그레이션)** | BP(당근·Adopt-a-Pet): 게시자 완료 표시 → 피드 정리·통지·성사추적                 |
| 상태 변경 UI   | ⋮ 메뉴만 / 상세 하단 CTA             | **상세 하단 CTA 소유자 분기 + ⋮ 보조**     | ⋮만은 발견성↓. 소유자는 문의하기 무의미 → 그 자리에 "입양완료 처리"(발견성↑)     |

### Open Issues (→ `/spec`서 데이터 계약 확정)

- TBD — 개인→공고 카드 공통 칩 집합 확정 + 공공 전용 D-day 칩 처리
- TBD — 개인 공고 상세 라우트·공유 URL·딥링크
- TBD — 통합 피드 정렬 일관화(공공 마감임박 ↔ 개인) — 기본 최신순 공통키
- TBD — 실종분실·입양생활 새 위치(후속)

(해소: 찜=개인 favorite 추가 / 상태=status 추가 + 상태 변경 UI 확정)
