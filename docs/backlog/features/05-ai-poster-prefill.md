...
> 상태: 발산 보류 (인프라 이동 우선). 실종분실 기능의 차별화 후보.
...
1. 작성 폼에서 이미지 촬영/첨부
2. 백엔드가 이미지(S3 URL)를 Vision LLM 에 전달 → 구조화 JSON 추출
...
- **keeper 이식**: 이미지가 이미 S3 presigned PUT 으로 업로드됨 → NestJS 모듈 1개가 S3 URL 을 Vision LLM 에 전달 → structured JSON → 프론트 검수. 저복잡도.

## 왜 keeper 에 맞나

- 앞서 컷한 "AI 얼굴인식 매칭"(자체 모델 학습 + 인프라)과 달리 **외부 API 단일 호출 + 기존 S3 흐름 재활용** → 인프라 가벼움.
