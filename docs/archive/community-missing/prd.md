...
- **PII 최소 노출** — 신고자 실명은 저장하지 않고, 전화번호는 화면에 표시하지 않으며 로그인 이용자의 연락 실행 시점에만 서버에서 전달한다.
...
- **callName 저장** — 신고자 실명은 발견자 연락에 불필요. 파이프라인 변환 단계에서 원천 드롭.
- **callTel 화면 표시** — 번호를 텍스트로 노출하거나 비인증 응답에 싣지 않는다. 저장은 하되 인증 이용자의 연락 실행 시점에만 전달한다.
...
- **FR-1. 공공 데이터 동기화 배치.** As keeper, I want lossInfo를 정기 동기화하여, so that 실종 공고가 자동 유지된다.
  ...
  - AC: 변환 단계에서 `callName` **드롭**(DB 진입 차단). `callTel`은 저장하되 응답 노출을 제한한다(FR-2).
...
- **FR-3. 상세 조회 API + 화면 + 공식 인계.** As 발견자, I want 상세 정보와 공식 채널 링크를, so that 주인 찾기에 기여한다.
  ...
  - AC: 상세 DTO에 `callName`·`callTel` 부재. 대신 `hasCallTel: boolean`. 실제 번호는 인증 필수 `GET /lost/:id/contact` 에서만 반환.
...
- **저장 안 함(원천 차단): `callName`.** `callTel`은 저장하되 비인증 응답에 싣지 않는다.
...
- 신규 모듈 `src/modules/lost/` — controller/service/dto:
  ...
  - `GET /lost/:id/contact` (연락처, 인증 필수).
  - 목록·상세 Response DTO에 `callName/callTel` 부재. 공식 게시판 URL 상수 포함.

### 영향 범위

- **PII 정책**: callName은 미보유. callTel은 공공데이터 원본을 그대로 보유하되, 노출 조건(isActive · 90일 창 · 인증)이 만족될 때만 반환한다. 동기화에서 빠지면 `isActive=false` 로 노출이 끊긴다.
...
| PII 처리      | 마스킹 노출 vs 고지 후 노출 vs 원천 미저장 | 인증 전용 반환   | 2026-07-10 번복. 아래 "PII 재검토" 참조                                                        |
...
**PII 재검토(2026-07-10):** `callTel` 원천 미저장 결정을 번복한다. 번호가 없으면 발견자가 보호자에게 연락할 수단이 사라져 기능의 목적 자체가 성립하지 않는다.

근거는 대법원 2016. 8. 17. 선고 2014다235080 — 이미 공개된 개인정보는 정보주체의 동의가 있었다고 객관적으로 인정되는 범위 내에서 별도 동의 없이 수집·이용·제공할 수 있다. 신고자가 반려동물을 찾을 목적으로 공개한 연락처를 발견자에게 연결하는 것은 그 범위 안이다.

따라서 저장은 하되 노출을 좁힌다. 상세 응답은 `hasCallTel: boolean` 만 내려주고, 실제 번호는 인증 필수 `GET /lost/:id/contact` 에서만 반환한다. 화면에 텍스트로 표시하지 않고 전화 연결에만 쓴다. 노출 조건은 `isActive` · 실종일 90일 이내 · `callTel not null` 세 가지를 모두 만족할 때다.

개인정보처리방침 5-1 절에 수집 출처·처리 목적·표시 최소화·정보주체의 삭제 요청 창구를 고지한다.
