...
const SheltersDetailTemplate = ({
  ...
  const renderListFooter = useCallback(() => {
    const { total, page, has_next } = adoptData || {};
    ...
    return (
      <View style={{ paddingBottom: 20 }}>
        {has_next && (
          <MoreButtonSection onFetch={onFetch} page={page + 1} total={totalPage} isLoading={isLoading.adopt} />
        )}
      </View>
    );
...
const MapSection = ({ shelterData }: MapSectionProps) => {
  ...
  const { camera, setCamera, mapRef, permissionStatus } = useMap(true);
