...
describe('useCreateComment', () => {
  it('mutate 호출 시 commentApi.create(postId, content) 가 호출된다', async () => {
    mockedCreate.mockResolvedValue({ id: 1 });
    const { wrapper } = setup();
    const { result } = renderHook(() => useCreateComment({ postId: 10 }), { wrapper });

    await act(async () => {
      await result.current.mutateAsync('첫 댓글');
    });

    expect(mockedCreate).toHaveBeenCalledWith(10, '첫 댓글');
  });
  ...
  it('성공 시 해당 postId 댓글 리스트 invalidate + 성공 토스트', async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync('내용');
    });
