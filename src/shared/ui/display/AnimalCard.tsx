...
import { theme } from '@/shared/model/constants/theme.constants';

import { NoImage, NoImageProps, Skeleton } from '../fallback';

export interface AnimalCardData<T> {
  id: AnnouncementValue['id'];
  uri: string;
  title: AnnouncementValue['fullName'];
  description: { label: string; value: string }[];
  chips?: ({ sort: number } & BasicCardChipsValue<T>)[];
}
export interface AnimalCardProps<T> extends NoImageProps {
...
const getChipStyle = (variant?: ChipVariant) => {
  const { error, success, notice, black } = theme.colors;
  ...
  const styles = {
    ...
    default: {
      containerStyle: { backgroundColor: '#F3F4F4' },
